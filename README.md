...
- 打包：PyInstaller `--onedir`（Windows 绿色文件夹形态）
...
目标系统：**Windows 10 / 11**。现代 Windows 上 WebView2 与 Edge/Chrome 通常随系统提供，构建出的 onedir 包即可直接运行，无需手动捆绑运行时：

```bash
# 用任意较新 Python（3.11+）冻结
...
工作流在 Windows runner 上用 Python 3.12 执行 PyInstaller，把 `API_Recorder` 文件夹打包为 `API_Recorder-<tag>-win64.zip` 并作为 Release 资产上传。

> 产物即为可在 Windows 10 / 11 直接运行的 onedir 包（依赖系统已装的 WebView2 / Edge）。如需更现代的 Python，同步升级工作流中的 `python-version` 与 `pyinstaller` 版本即可。
...
- 控制面板 UI 内核为 WebView2（需 Windows 10 / 11 已装的 WebView2 或 Edge 运行时）。
- 抓包依赖系统可用浏览器（Edge / Chrome）；未随包捆绑特定版本浏览器。
- 目标系统为 Windows 10 / 11，不再支持 Windows 7。
